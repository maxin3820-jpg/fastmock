    ...
    <nav className="sticky top-0 z-50 border-b border-white/5"
      ...
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        <Link href="/" className="flex items-center gap-2 font-bold text-lg">
          <GraduationCap className="w-6 h-6" style={{ color: 'var(--cyan)' }} />
      ...
      {open && (
        <div className="md:hidden border-t border-white/5 px-4 py-4 flex flex-col gap-4"
          ...
          <Link href="/register" className="btn-primary text-center" onClick={() => setOpen(false)}>
            Get Access — PKR 300
          </Link>
        </div>
      )}
