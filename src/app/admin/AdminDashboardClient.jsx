...
import { Users, Settings, Upload, BarChart2, LogOut, ShieldCheck, Search, CheckSquare, Square, Check, RefreshCw } from 'lucide-react'
...
export default function AdminDashboardClient({ stats, initialConfig }) {
  ...
  const filteredUsers = users.filter(u => {
    const matchSearch = search === '' || u.full_name.toLowerCase().includes(search.toLowerCase()) || u.email.toLowerCase().includes(search.toLowerCase()) || u.whatsapp_number.includes(search)
  ...
  const bulkApprove = async () => {
    ...
    const res = await fetch('/api/admin/users/approve', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: Array.from(selected) }) })
    const d = await res.json()
    setActionMsg(d.message || 'Done')
    await loadUsers(); clearSelection(); setActionLoading(false)
  ...
  const grantRevoke = async (revoke = false) => {
    ...
    const res = await fetch('/api/admin/users/grant', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: grantEmail.trim(), revoke }) })
    const d = await res.json()
    setActionMsg(d.message || (revoke ? 'Access revoked' : 'Access granted'))
    await loadUsers(); setGrantEmail(''); setGrantLoading(false)
    setTimeout(() => setActionMsg(''), 3000)
  }
  ...
  const saveConfig = async () => {
    ...
    setConfigSaving(false); setActionMsg('Config saved!')
    setTimeout(() => setActionMsg(''), 3000)
  }
  ...
  const uploadQuiz = async () => {
    ...
    const fd = new FormData(); fd.append('file', uploadFile); fd.append('testId', uploadTestId)
        ...
        <div className="flex min-w-max">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={{ borderColor: tab === t.id ? 'var(--cyan)' : 'transparent', color: tab === t.id ? 'var(--cyan)' : '#9ca3af' }}>
              {t.icon}{t.label}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={loadUsers} className="btn-navy text-sm p-2" style={{ minHeight: 'auto' }}><RefreshCw className="w-4 h-4" /></button>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="flex gap-2">
                <input value={grantEmail} onChange={e => setGrantEmail(e.target.value)} className="input-field flex-1 text-sm" style={{ minHeight: '40px' }} placeholder="student@email.com" />
                ...
                <button onClick={() => grantRevoke(true)} disabled={grantLoading} className="text-sm px-4 rounded-xl font-semibold" style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.25)', minHeight: '40px', padding: '0 1rem' }}>Revoke</button>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} className="input-field pl-10 text-sm" style={{ minHeight: '40px' }} placeholder="Search by name, email, or WhatsApp..." />
              </div>
              <div className="flex gap-2">
                {['all', 'premium', 'pending'].map(f => (
                  <button key={f} onClick={() => setFilter(f)} className="px-3 py-2 rounded-lg text-xs font-semibold capitalize"
                    style={{ background: filter === f ? 'rgba(0,180,216,0.15)' : 'rgba(255,255,255,0.04)', color: filter === f ? 'var(--cyan)' : '#9ca3af', border: `1px solid ${filter === f ? 'rgba(0,180,216,0.3)' : 'rgba(255,255,255,0.07)'}` }}>
                    {f}
                  </button>
                ))}
              </div>
            ...
            {filteredUsers.length > 0 && (
              <div className="flex items-center gap-3 text-sm text-gray-400">
                <button onClick={selected.size === filteredUsers.length ? clearSelection : selectAll} className="flex items-center gap-1.5 hover:text-white">
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {filteredUsers.length === 0 ? (
                      ...
                      <tr key={u.id} onClick={() => toggleSelect(u.id)} className="cursor-pointer hover:bg-white/[0.02]"
                        ...
                        <td className="px-4 py-3" onClick={e => { e.stopPropagation(); toggleSelect(u.id) }}>
                          {selected.has(u.id) ? <CheckSquare className="w-4 h-4" style={{ color: 'var(--cyan)' }} /> : <Square className="w-4 h-4 text-gray-600" />}
                        </td>
                        ...
                        <td className="px-4 py-3"><span className={`badge ${u.is_premium ? 'badge-success' : 'badge-warning'}`}>{u.is_premium ? 'Premium' : 'Pending'}</span></td>
          ...
          <div className="space-y-4">
            ...
            <div className="card space-y-4" style={{ padding: '1.5rem' }}>
              {[
                ...
                { key: 'whatsapp_number', label: 'WhatsApp Number (no +)', type: 'text' },
                ...
                <div key={f.key}>
                  ...
                  {f.type === 'textarea' ? (
                    <textarea value={config[f.key] || ''} onChange={e => setConfig(c => ({ ...c, [f.key]: e.target.value }))} className="input-field resize-none" rows={3} style={{ height: 'auto' }} />
                  ) : (
                    <input value={config[f.key] || ''} onChange={e => setConfig(c => ({ ...c, [f.key]: e.target.value }))} className="input-field" type="text" />
                  )}
                </div>
              ))}
          ...
          <div className="space-y-4">
            ...
            <div className="card" style={{ background: 'rgba(0,180,216,0.05)', borderColor: 'rgba(0,180,216,0.2)' }}>
                    ...
                    <tr>
                      {['Question','Option A','Option B','Option C','Option D','Correct Answer','Subject'].map(h => (
                        <th key={h} className="px-3 py-2 text-left font-semibold whitespace-nowrap"
                          style={{ background: 'rgba(0,180,216,0.15)', color: 'var(--cyan)', border: '1px solid rgba(0,180,216,0.2)' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      {['If 2x+3=7, find x','1','2','3','4','B','Advanced Math'].map((v, i) => (
                        <td key={i} className="px-3 py-2 text-gray-400 whitespace-nowrap" style={{ border: '1px solid rgba(255,255,255,0.05)' }}>{v}</td>
                      ))}
                    </tr>
                  </tbody>
                </table>
              ...
              <p className="text-xs text-gray-500 mt-3">
                Subjects: <code style={{ color: 'var(--cyan)' }}>Advanced Math, Basic Math, Analytical Reasoning, English</code><br />
                Correct Answer: <code style={{ color: 'var(--cyan)' }}>A, B, C, or D</code>
              </p>
              <a href="/sample-quiz-template.csv" download className="inline-flex items-center gap-1.5 mt-3 text-sm font-medium" style={{ color: 'var(--cyan)' }}>
            ...
            <div className="card space-y-4" style={{ padding: '1.5rem' }}>
              <div>
                ...
                <select value={uploadTestId} onChange={e => setUploadTestId(e.target.value)} className="input-field" style={{ background: 'rgba(10,22,40,0.8)' }}>
              ...
              {uploadMsg && (
                <div className="rounded-lg p-3 text-sm" style={{ background: uploadMsg.includes('success') ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: uploadMsg.includes('success') ? '#10b981' : '#ef4444', border: '1px solid' }}>
                  {uploadMsg}
                </div>
              )}
              <button onClick={uploadQuiz} disabled={uploading || !uploadFile || !uploadTestId} className="btn-primary">
                {uploading ? <><div className="spinner w-4 h-4" />Uploading...</> : <><Upload className="w-4 h-4" />Upload Questions</>}
              </button>
            </div>
          </div>
        )}
