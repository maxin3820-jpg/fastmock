...
import { CheckCircle, Clock, BarChart2, RefreshCw, Zap, ShieldCheck, BookOpen, Target, ChevronRight, MessageCircle, Star, AlertTriangle } from 'lucide-react'
...
export default async function HomePage() {
  ...
  const features = [
    { icon: <Target className="w-5 h-5" />, title: 'Exact FAST Pattern', desc: 'Questions mapped 1:1 to real FAST entry test format.' },
    { icon: <AlertTriangle className="w-5 h-5" />, title: 'Negative Marking', desc: '-0.25 Math/IQ, -0.0825 English. Exactly like the real exam.' },
    { icon: <BookOpen className="w-5 h-5" />, title: '300+ Premium MCQs', desc: '3 full-length tests covering every section.' },
    { icon: <RefreshCw className="w-5 h-5" />, title: 'Unlimited Retakes', desc: 'Practice until confident. No limits, no extra charges.' },
    { icon: <Clock className="w-5 h-5" />, title: 'Timer Mode', desc: 'Simulate real exam pressure with a 2-hour countdown.' },
    { icon: <BarChart2 className="w-5 h-5" />, title: 'Detailed Analytics', desc: 'Section-wise breakdown: see exactly where you lose marks.' },
    { icon: <ShieldCheck className="w-5 h-5" />, title: 'Auto-Save Progress', desc: 'Phone died? Internet dropped? Your answers are safe.' },
    ...
    <div className="min-h-screen gradient-cyber">
      ...
      <section className="relative hero-pattern pt-16 pb-20 px-4 text-center overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto fade-in">
          ...
          <p className="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto mb-10 leading-relaxed">{subtext}</p>
      ...
      <section className="py-16 px-4">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              { n: '1', t: 'Sign Up Free', d: 'Create your account with name, WhatsApp, email, and university.' },
      ...
      <section className="py-20 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <div className="card glow-cyan text-center"
            style={{ background: 'linear-gradient(135deg, rgba(0,119,182,0.15), rgba(0,180,216,0.08))' }}>
            <h2 className="text-3xl font-extrabold text-white mb-3">Ready to Ace FAST?</h2>
            <p className="text-gray-300 mb-8">Join hundreds of students scoring higher with our premium mock tests.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a href={waUrl} target="_blank" rel="noopener noreferrer" className="btn-primary text-lg px-8 py-4 glow-cyan">
                <MessageCircle className="w-5 h-5" />Buy Now — {price} Only
              </a>
              <Link href="/register" className="btn-secondary px-8 py-4">Create Free Account</Link>
            </div>
