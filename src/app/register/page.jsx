...
export default function RegisterPage() {
  ...
  const submit = async (e) => {
    ...
    const { error: authErr } = await supabase.auth.signUp({
      ...
      options: {
        data: { full_name: form.full_name, whatsapp_number: form.whatsapp_number, university: form.university },
      },
    })
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card" style={{ padding: '2rem' }}>
          <form onSubmit={submit} className="space-y-4">
            {[
              ...
              <div key={f.name}>
                ...
                <input name={f.name} value={form[f.name]} onChange={handle} required className="input-field" type={f.type} placeholder={f.placeholder} />
              </div>
            ))}
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1.5">Password *</label>
              <div className="relative">
                <input name="password" value={form.password} onChange={handle} required className="input-field pr-12" placeholder="Min. 8 characters" type={showPw ? 'text' : 'password'} minLength={8} />
          ...
          <div className="mt-6 pt-6 border-t text-center" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
            ...
            <a href={`https://wa.me/923036326202?text=${encodeURIComponent('Hi! I want to buy the 3 FAST Mock Tests. Please share your JazzCash/EasyPaisa details so I can send the receipt screenshot.')}`}
              target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-semibold" style={{ color: '#25D366' }}>
        ...
        <p className="text-center text-gray-400 text-sm mt-6">
          ...
          <Link href="/login" className="font-semibold hover:underline" style={{ color: 'var(--cyan)' }}>Login here</Link>
        </p>
      </div>
    </div>
  )
