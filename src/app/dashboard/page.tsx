...
export default async function DashboardPage() {
  ...
  const bestScore = attempts && attempts.length > 0
    ? Math.max(...attempts.map((a: { total_score: number }) => a.total_score))
    : null
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {isPremium ? (
              ...
              <span className="badge badge-warning">Pending Access</span>
            )}
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        {!isPremium && (
          <div className="card text-center" style={{ background: 'rgba(245,158,11,0.08)', borderColor: 'rgba(245,158,11,0.25)' }}>
            ...
            <p className="text-gray-400 text-sm mb-4">
              Your account is created! Send payment via WhatsApp and we'll unlock your access within minutes.
            </p>
            <a href={waUrl} target="_blank" rel="noopener noreferrer" className="btn-primary inline-flex">
              <MessageCircle className="w-4 h-4" />
              Pay PKR 300 on WhatsApp
            </a>
          </div>
        )}
        ...
        <div>
          ...
          <div className="space-y-4">
            {tests?.map((test: { id: string; test_number: number; title: string; description: string; total_questions: number; duration_minutes: number }) => {
              const testAttempts = attempts?.filter((a: { test_id: string }) => a.test_id === test.id) || []
              const bestTestScore = testAttempts.length > 0
                ? Math.max(...testAttempts.map((a: { total_score: number }) => a.total_score))
                : null
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-3 mt-3 text-xs text-gray-400">
                        ...
                        {bestTestScore !== null && (
                          <span style={{ color: 'var(--cyan)' }}>Best: {formatScore(bestTestScore)} pts</span>
                        )}
                    ...
                    <div className="flex flex-col gap-2 flex-shrink-0">
                      ...
                      {testAttempts.length > 0 && (
                        <Link href={`/test/${test.id}/results`}
                          className="text-center text-xs font-medium py-1.5 px-3 rounded-lg hover:bg-white/5 transition-colors"
        ...
        {attempts && attempts.length > 0 && (
          <div>
            ...
            <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
              {attempts.slice(0, 5).map((a: { id: string; test_id: string; submitted_at: string; total_score: number; correct_count: number; incorrect_count: number; mode: string }) => {
                const t = tests?.find((x: { id: string }) => x.id === a.test_id)
                return (
                  <div key={a.id} className="flex items-center gap-4 px-5 py-4 border-b hover:bg-white/[0.02] transition-colors"
                    style={{ borderColor: 'rgba(255,255,255,0.04)' }}>
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white text-sm">{t?.title || 'Mock Test'}</p>
                      <p className="text-xs text-gray-500 mt-0.5">{a.submitted_at ? formatDate(a.submitted_at) : '—'} • {a.mode === 'timer' ? '⏱ Timer' : '📖 Practice'}</p>
                    </div>
                    ...
                    <Link href={`/test/${a.test_id}/results?attempt=${a.id}`}
                      className="text-xs text-gray-400 hover:text-white ml-2 hidden sm:block">Details →</Link>
                  </div>
                )
              })}
            </div>
